...
export const Config: Schema<Config> = Schema.object({
  ...
  upscaler: Schema.string().default('SwinIR_4x').description('默认放大模型'),
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.command(name, 'sd-extra')
    ...
    .option('upscaler', '-u <upscaler:text> 单次选择放大模型')
    .option('switch', '-x <switch:boolean> 切换默认放大模型')
    .option('show', '-s <show:boolean> 显示可用放大模型')
    .action(async ({ session, options }, input) => {
      ...
      if (options.switch) {
        ...
        session.send(`当前放大模型为：${options.upscaler}\n可用的放大模型有：\n${upscalerList}\n请输入要切换的放大模型序号：`)
        ...
        return `已将默认放大模型切换为 ${upscalers[index - 1]}`
      }
      if (options.show) return `当前放大模型为：${options.upscaler}\n可用的放大模型有：\n${(await getUpscalers()).join('\n')}`
